...
#create action
    ...
    def __generateActions(self, players):
        ...
        for p1 in players:
            ...
            self.actions[1].append(p1 + ". Ex or Kai")
...
class SoundCreator(threading.Thread):
    ...
    def __play(self, phrase):
        subprocess.call(["pico2wave", "--wave=" + tmpDir + "/tmpSaufen.wav", "\"" + phrase + "\""], stdout = f, stderr = subprocess.STDOUT)
        subprocess.call(["mplayer", tmpDir + "/tmpSaufen.wav"], stdout = f, stderr = subprocess.STDOUT)
        os.remove(tmpDir + "/tmpSaufen.wav")

#find tmp directory
if os.path.isdir(str(Path.home()) + "/tmp"):
    #location specific for my system
...
sc = SoundCreator(gf,1 , shutdownEvent)
sc.start()
